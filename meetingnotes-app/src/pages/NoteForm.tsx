...
const NoteForm: React.FC<NoteFormProps> = ({ open, note, onClose, onSave }) => {
  ...
  const handleSave = () => {
    ...
    const newNote: Note = {
      ...
      content,
      actionItems,
      created: note?.created || currentDate, // Use existing created date if editing, or set to current date if new
      updated: currentDate,
      _id: ''
    };
    onSave(newNote);
    ...
    <Dialog open={open} onClose={onClose}>
      ...
      <DialogContent>
        ...
        {actionItems.map((item, index) => (
          <FormGroup key={index}>
            <FormControlLabel
              control={<Checkbox checked={item.completed} onChange={(e) => handleActionItemChange(index, e)}sx={{ '&.Mui-checked': { color: 'purple' } }} />}
              label={<TextField fullWidth value={item.text} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleActionItemTextChange(index, e)}sx={{ color: 'purple' }} />}
            />
          </FormGroup>
        ))}
        <Button onClick={addActionItem} color="primary" sx={{ color: 'purple' }}>Add Action Item</Button>
        </DialogContent>
      <DialogActions>
        <Button onClick={onClose} sx={{ color: 'purple' }}>Cancel</Button>
        <Button onClick={handleSave} sx={{ color: 'purple' }}>Save</Button>
      </DialogActions>
    </Dialog>
  );
};
